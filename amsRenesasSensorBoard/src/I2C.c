...
ssp_err_t I2CWriteRegister(uint8_t slaveAddress, uint8_t registerAddress, uint8_t data, bool const restart)
{
    ...
    return I2CWrite (slaveAddress, (uint8_t * const ) slaveData, 2, restart);
}
...
{
    ssp_err_t error;
    error = I2CWriteByte(slaveAddress, registerAddress, true);
    if (error != SSP_SUCCESS)
         return error;

    error = I2CRead(slaveAddress, data, length, restart);
    if (error != SSP_SUCCESS)
        return error;
